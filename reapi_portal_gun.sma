...
new const PLUGIN_URL[] = "https://dev-cs.ru/resources/1523/";
...
new const PORTAL_CLASSNAME[] = "pg_ent";
...
@create_portal_pre(pPlayer, bool: bIsPrimaryAttack)
{
    ...
    if(!@is_get_origin_aiming_place_accuracy(pPlayer, vecOrigin, vecAngles, vecOriginTeleport, vecAnglesTeleport))
    {
...
@create_portal_post(pPlayer, Float: vecOrigin[3], Float: vecAngles[3], Float: vecOriginTeleport[3], Float: vecAnglesTeleport[3], bool: bIsPrimary)
{
    ...
    set_entvar(eEnt, var_classname, PORTAL_CLASSNAME);
...
@portal_touch(eEnt, pPlayer)
{
    ...
    if(fNextTime < fGameTime)
    {
        ...
        if(is_user_connected(pOwner))
        {
            ...
            new Float: vecAnglesTeleport[3];
            get_entvar(ePortalEnt, var_vuser2, vecAnglesTeleport);
            vecAnglesTeleport[2] = 0.0;

            set_entvar(pPlayer, var_origin, vecOriginTeleport);
            set_entvar(pPlayer, var_v_angle, vecAnglesTeleport);
            set_entvar(pPlayer, var_angles, vecAnglesTeleport);
...
bool: @is_get_origin_aiming_place_accuracy(pPlayer, Float: vecOrigin[3], Float: vecAngles[3], Float: vecOriginTeleport[3], Float: vecAnglesTeleport[3])
{
    ...
    while((eFindEnt = engfunc(EngFunc_FindEntityInSphere, eFindEnt, vecOrigin, 30.0)))
    {
        if(is_user_connected(eFindEnt) || (!is_nullent(eFindEnt) && FClassnameIs(eFindEnt, PORTAL_CLASSNAME)))
            return false;
    }
...
@show_hud_info(pPlayer)
{
    set_hudmessage(0, 255, 0, -1.0, 0.25, .holdtime = 5.0);
